...
def main(WHICHSAMPLES,wf,CORES,callsdatabase=None):
	...
	if wf=='Y':
		...
		for db in callsdatabase:
			appdb=[]
			for app in db:#[sample1] [2] [3]...
				Packetsfile=[]
				for line in app:#[apiL1,apiL2....]
					Packetsline=[]
					for j in line.split('\t')[:-1]:
						match = False
						for y in PACKETS:
							#x = y.partition('.')[2] #split, so x is empty ''
							x = y 
						...
						if match == False:
							...
							if obfcount>=len(splitted)/2:
								...
								match='selfdefined'
						Packetsline.append(match) #append  api at line j
					Packetsfile.append(Packetsline) #append api 
